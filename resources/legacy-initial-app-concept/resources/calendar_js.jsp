...
{
  var url = 'https://ponorez.online/reservation/';
  if (/reservation[^/]*\/$/.test(url))
  {
    // Leave as is.
  }
  else if (/perfecthawaiivacationguide.com$/.test(window.location.hostname) || /^(\w+\.)*ponorez\.online$/.test(window.location.hostname) || /^(\w+\.)*hawaiifun\.org$/.test(window.location.hostname) || /^(\w+\.)*hawaiifunfusion\.com/.test(window.location.hostname))
  {
    url = 'https://ponorez.online/reservation/';
  }
  else
  {
    url += 'reservation/';
...
window.showAvailabilityCalendar2 = function(activityId, fieldId, options) {
  ...
  var checkAutoAttached = (options['checkAutoAttached'] != null ? options['checkAutoAttached'] : true);
  ...
  function resolveIdOrElement(target)
  {
    return (
        typeof(target) === 'string' ? $('#' + target)
        : (target && typeof(target) === 'object' && ('jquery' in target)) ? $(target.get(0))
        // ^^^ This could be an "alien" jQuery object, so unpack it.
        // 'target' condition filters out null (which weirdly have typeof == 'object').
        : $(target).first()
    );
  }

  var $field = resolveIdOrElement(fieldId);
  if ($field.length == 0) return;

  var $anchor = resolveIdOrElement(anchorId);
  if ($anchor.length == 0) $anchor = $field;
  ...
  function makeCacheIndex(year_month)
  {
    return local
        + '_' + activityIdStr + '_' + agencyId
        + '_' + blocksOnly + '_' + checkAutoAttached + '_' + webBooking + '_' + hawaiifunBooking + '_' + agencyBooking
        + '_' + year_month + '_' + minAvailabilityStr;
  }
  ...
  $panel.datepicker_async({
    ...
    onPrepareMonthBegin: function (year, month, inst) {
      ...
      for (var i = 0, y = year, m = month; i < numberOfMonths; ++i, ++m)
      {
        ...
        if (!(makeCacheIndex(y + '_' + m) in reservationcalendar_availability))
        {
          yms.push(y + '_' + m);
        }
      }
      ...
      $.ajax({
        ...
        data: {
          ...
          checkAutoAttached: checkAutoAttached,
      ...
      .done(function(data) {
        ...
        {
          reservationcalendar_availability[makeCacheIndex(yms[i])] = data['yearmonth_' + yms[i]];
        }
        deferred.resolve();
      })
