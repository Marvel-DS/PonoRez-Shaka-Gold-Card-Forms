...
<header class="flex flex-col md:flex-row px-4 md:px-6 py-6 gap-4 items-center md:justify-between text-center md:text-left" data-component="form-header" role="banner">
    ...
    <?php if (!empty($homeLink['enabled']) && !empty($homeLink['label']) && !empty($homeLink['url'])): ?>
        ...
        <a href="<?= htmlspecialchars($homeLink['url'], ENT_QUOTES, 'UTF-8') ?>"
           ...
           aria-label="View all discounted activities for Rappel Maui <?= htmlspecialchars($supplierName, ENT_QUOTES, 'UTF-8') ?>">
